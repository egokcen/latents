...
def simulation_params():
    ...
    obs_params = ObsParamsARD.generate(y_dims, x_dim, hyper_priors, snr, np.random.default_rng(seed=42))
    gp_params = RBF_GP_Params(x_dim=x_dim, num_groups=num_groups, T=T, eps=eps, gamma=gamma, D=D)
...
def test_generate_observations(simulation_params):
    ...
    for group_idx, y_dim in enumerate(y_dims):
        ...
        assert np.allclose(d_g, empirical_mean_g, atol=1e-2), (
            f"Empirical mean for group {group_idx} does not match expected mean"
        )

        # Compute empirical covariance
        observations_g_flat = np.reshape(observations_g,(y_dims[group_idx], -1))  # Shape: (N_samples, yDims_g)
        empirical_cov_g = np.cov(observations_g_flat)

        # Compute theoretical covariance
        C_g = Cs[group_idx]
        Phi_g_inv = np.diag(1.0 / phis[group_idx])
        Cov_x_t = np.eye(state_params.x_dim)  # Simplification: assume Cov[x_t] is identity
        theoretical_cov_g = C_g @ Cov_x_t @ C_g.T + Phi_g_inv

        assert np.allclose(empirical_cov_g, theoretical_cov_g, atol=1e-2), (
            f"Empirical covariance for group {group_idx} does not match theoretical covariance"
        )

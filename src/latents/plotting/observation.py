...
def plot_dimensionalities(
    ...
    sem_dim
        `ndarray` of `float`, shape ``(n_dim_types,)``, optional.
        Standard error of the mean for each dimension type.
    group_names
...
def plot_var_exp(
    ...
    dim_types
        ...
        Binary array indicating which groups are involved in each dimension type.
    sem_var_exp
        `ndarray` of `float`, shape ``(n_groups, n_dim_types)``, optional.
...
def plot_dims_pairs(
    ...
    ----------
    pair_dims
        `ndarray` of `float`, shape ``(n_pairs, 3)``.
        Dimensionalities for each pair: [total_group1, shared, total_group2].
    pairs
        `ndarray` of `int`, shape ``(n_pairs, 2)``.
        Indices of groups in each pair.
    n_groups
        Total number of groups.
    sem_pair_dims
        `ndarray` of `float`, shape ``(n_pairs, 3)``, optional.
        Standard error of the mean for pairwise dimensionalities.
    group_names
        List of group names for labeling. If None, uses "1", "2", etc.
    fig
        Figure on which to draw. If None, uses current figure.
...
def plot_var_exp_pairs(
    ...
    ----------
    pair_var_exp
        `ndarray` of `float`, shape ``(n_pairs, 2)``.
    ...
    n_groups
        Total number of groups.
    sem_pair_var_exp
        `ndarray` of `float`, shape ``(n_pairs, 2)``, optional.
        Standard error of the mean for pairwise variance explained.
    group_names
        List of group names for labeling. If None, uses "1", "2", etc.
    fig
        Figure on which to draw. If None, uses current figure.
